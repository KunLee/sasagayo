...
export default function Header() {
  ...
  return <header className="sticky top-0 z-50 border-b border-stone-900/8 bg-[#f7f4ee]/88 backdrop-blur-xl">
    <div className="mx-auto flex h-18 max-w-[1400px] items-center gap-8 px-5 sm:px-8">
      ...
      <nav className="hidden items-center gap-1 rounded-full bg-stone-900/4 p-1 lg:flex" aria-label="Main navigation"><Link className="nav-pill nav-pill-active" href="/"><Compass className="size-3.5"/>Discover</Link><Link className="nav-pill" href="/#stories" onClick={(event)=>navigateToSection(event,'stories')}>Stories</Link><Link className="nav-pill" href="/#circles" onClick={(event)=>navigateToSection(event,'circles')}>Circles</Link></nav>
